...
public class CloneGraph {
    ...
    public Node cloneGraph(Node node) {
        ...
        dfs(visited, node, clone);


        return clone;
    }

    private void dfs(Map<Integer, Node> visited, Node node, Node clone) {
        clone.val = node.val;
        visited.put(clone.val, clone);
        List<Node> cloneNeighbors = new ArrayList<>(node.neighbors.size());
        for (Node current : node.neighbors) {
            Node cloneCurrent = visited.get(current.val);
            if (cloneCurrent == null) {
                cloneCurrent = new Node(current.val);
                dfs(visited, current, cloneCurrent);
            }

            cloneNeighbors.add(cloneCurrent);
        }
